...
- Project 01 (Cost-Effective Heat Mitigation) shows 4 images (2 large + 2 gallery);
  Project 02 (Green-Roof Optimization) shows 1; the Mixture paper (Project 03) shows 2;
  the UN project shows its proposal-cover illustration.
- The homepage About section includes a 6-photo strip (`images/profile/full/01–06.jpg`)
  in a fixed order; photos 1 and 3 are in color, the rest (2, 4, 5, 6) were converted to
  grayscale during processing.
- Accent color was shifted from the original vermilion (#ff5a2e) to a warm brick red
  (#d9382b), sampled from a reference moodboard image — this is set once in
  `css/style.css` (`--accent` / `--accent-dim`) and in `js/main.js` (hero canvas hot color).
- "Confront" (fine-art/06) has been removed from the site entirely per request.
